...
function shadowLength(madhab) {
	switch (madhab) {
		case Madhab.Shafi: return 1;
		case Madhab.Hanafi: return 2;
		default: throw "Invalid Madhab";
	}
}
//#endregion
...
function dayOfYear(date) {
	...
	const months = [
		31,
		Astronomical.isLeapYear(date.getFullYear()) ? 29 : 28,
		31,
		30,
		31,
		30,
...
var Astronomical = {
	...
	ascendingLunarNodeLongitude(julianCentury) {
		...
		const term3 = .0020708 * Math.pow(T, 2);
	...
	solarEquationOfTheCenter(julianCentury, meanAnomaly) {
		...
		const term1 = (1.914602 - .004817 * T - 14e-6 * Math.pow(T, 2)) * Math.sin(Mrad);
		const term2 = (.019993 - 101e-6 * T) * Math.sin(2 * Mrad);
		const term3 = 289e-6 * Math.sin(3 * Mrad);
		return term1 + term2 + term3;
	},
	apparentSolarLongitude(julianCentury, meanLongitude) {
		const T = julianCentury;
		const longitude = meanLongitude + Astronomical.solarEquationOfTheCenter(T, Astronomical.meanSolarAnomaly(T));
		const Omega = 125.04 - 1934.136 * T;
		return unwindAngle(longitude - .00569 - .00478 * Math.sin(degreesToRadians(Omega)));
	},
	meanObliquityOfTheEcliptic(julianCentury) {
		const T = julianCentury;
		const term1 = 23.439291;
		const term2 = .013004167 * T;
	...
	apparentObliquityOfTheEcliptic(julianCentury, meanObliquityOfTheEcliptic) {
		...
		return Epsilon0 + .00256 * Math.cos(degreesToRadians(O));
	},
	...
	nutationInLongitude(julianCentury, solarLongitude, lunarLongitude, ascendingNode) {
		...
		const term3 = .23 / 3600 * Math.sin(2 * degreesToRadians(Lp));
		const term4 = .21 / 3600 * Math.sin(2 * degreesToRadians(Omega));
		return term1 - term2 - term3 + term4;
	},
	nutationInObliquity(julianCentury, solarLongitude, lunarLongitude, ascendingNode) {
		...
		const term2 = .57 / 3600 * Math.cos(2 * degreesToRadians(L0));
		const term3 = .1 / 3600 * Math.cos(2 * degreesToRadians(Lp));
		const term4 = .09 / 3600 * Math.cos(2 * degreesToRadians(Omega));
		return term1 + term2 + term3 - term4;
	},
	...
	approximateTransit(longitude, siderealTime, rightAscension) {
		...
		return normalizeToScale((rightAscension + L * -1 - Theta0) / 360, 1);
	},
	...
	correctedHourAngle(approximateTransit, angle, coordinates, afterTransit, siderealTime, rightAscension, previousRightAscension, nextRightAscension, declination, previousDeclination, nextDeclination) {
		...
		const H = Theta - Lw - a;
		return (m + (Astronomical.altitudeOfCelestialBody(coordinates.latitude, delta, H) - h0) / (360 * Math.cos(degreesToRadians(delta)) * Math.cos(degreesToRadians(coordinates.latitude)) * Math.sin(degreesToRadians(H)))) * 24;
	},
...
var aqrabYaumResolver = (coordinates, date, daysAdded = 1, direction = 1) => {
	...
	if (!isValidSolarTime(solarTime) || !isValidSolarTime(tomorrowSolarTime)) return aqrabYaumResolver(coordinates, date, daysAdded + (direction > 0 ? 0 : 1), -direction);
	return {
		date,
		tomorrow,
		coordinates,
...
var aqrabBaladResolver = (coordinates, date, latitude) => {
	...
	if (!isValidSolarTime(solarTime) || !isValidSolarTime(tomorrowSolarTime)) return Math.abs(latitude) >= UNSAFE_LATITUDE ? aqrabBaladResolver(coordinates, date, latitude - Math.sign(latitude) * LATITUDE_VARIATION_STEP) : null;
	return {
		date,
		tomorrow,
...
var polarCircleResolvedValues = (resolver, date, coordinates) => {
	...
	switch (resolver) {
		case PolarCircleResolution.AqrabYaum: return aqrabYaumResolver(coordinates, date) || defaultReturn;
...
var CalculationParameters = class {
	...
	nightPortions() {
		switch (this.highLatitudeRule) {
			case HighLatitudeRule.MiddleOfTheNight: return {
				fajr: 1 / 2,
				isha: 1 / 2
			};
			case HighLatitudeRule.SeventhOfTheNight: return {
				fajr: 1 / 7,
				isha: 1 / 7
			};
			case HighLatitudeRule.TwilightAngle: return {
				fajr: this.fajrAngle / 60,
				isha: this.ishaAngle / 60
			};
			default: throw `Invalid high latitude rule found when attempting to compute night portions: ${this.highLatitudeRule}`;
		}
	}
};
//#endregion
...
var PrayerTimes = class {
	constructor(coordinates, date, calculationParameters) {
		...
		const safeFajr = function() {
			if (calculationParameters.method === "MoonsightingCommittee") return Astronomical.seasonAdjustedMorningTwilight(coordinates.latitude, dayOfYear(date), date.getFullYear(), sunriseTime);
			else {
		...
		if (calculationParameters.ishaInterval > 0) ishaTime = dateByAddingMinutes(sunsetTime, calculationParameters.ishaInterval);
		else {
			...
			const safeIsha = function() {
				if (calculationParameters.method === "MoonsightingCommittee") return Astronomical.seasonAdjustedEveningTwilight(coordinates.latitude, dayOfYear(date), date.getFullYear(), sunsetTime, calculationParameters.shafaq);
				else {
		...
		if (calculationParameters.maghribAngle) {
			...
			if (sunsetTime < angleBasedMaghrib && ishaTime > angleBasedMaghrib) maghribTime = angleBasedMaghrib;
		}
...
//#endregion
//#region node_modules/adhan/lib/esm/Qibla.js
function qibla(coordinates) {
	...
	return unwindAngle(radiansToDegrees(Math.atan2(term1, term2 - term3)));
}
//#endregion
//#region node_modules/adhan/lib/esm/SunnahTimes.js
var SunnahTimes = class {
	constructor(prayerTimes) {
		const date = prayerTimes.date;
		const nextDay = dateByAddingDays(date, 1);
		const nightDuration = (new PrayerTimes(prayerTimes.coordinates, nextDay, prayerTimes.calculationParameters).fajr.getTime() - prayerTimes.maghrib.getTime()) / 1e3;
